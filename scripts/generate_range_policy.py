# -*- coding: utf-8 -*-
# MIA_IA_SYSTEM_v2_2025/src/scripts/generate_range_policy.py
# Script pour générer la politique SAC pour le mode range de MIA_IA_SYSTEM_v2_2025.
#
# Version : 2.1.3
# Date : 2025-05-13
#
# Rôle : Crée une politique SAC initiale pour le trading ES en mode range (Phase 10).
...
#
# Outputs : src/model/router/policies/range_policy.pkl,
#           data/logs/market/generate_range_policy.log,
#           data/logs/generate_range_policy_performance.csv,
#           data/range_policy_snapshots/snapshot_*.json.gz
#
# Notes :
...
# - Tests unitaires dans tests/test_generate_range_policy.py.
...
POLICY_PATH = os.path.join(
    BASE_DIR, "src", "model", "router", "policies", "range_policy.pkl"
)
LOG_DIR = os.path.join(BASE_DIR, "data", "logs", "market")
SNAPSHOT_DIR = os.path.join(BASE_DIR, "data", "range_policy_snapshots")
CSV_LOG_PATH = os.path.join(
    BASE_DIR, "data", "logs", "generate_range_policy_performance.csv"
)

# Configuration du logging
os.makedirs(LOG_DIR, exist_ok=True)
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "generate_range_policy.log"),
    level=logging.INFO,
...
class RangePolicyGenerator:
    """
    Classe pour générer et sauvegarder une politique SAC pour le mode range.
    """
    ...
    def __init__(self, config_path: Path = Path(CONFIG_PATH)):
        ...
        try:
            ...
            miya_speak(
                "RangePolicyGenerator initialisé",
                tag="RANGE_POLICY",
                voice_profile="calm",
                priority=2,
            )
            AlertManager().send_alert("RangePolicyGenerator initialisé", priority=1)
            logger.info("RangePolicyGenerator initialisé")
            self.log_performance("init", 0, success=True)
            self.save_snapshot("init", {"config_path": str(self.config_path)})
        except Exception as e:
            error_msg = f"Erreur initialisation RangePolicyGenerator: {str(e)}\n{traceback.format_exc()}"
            miya_alerts(
                error_msg, tag="RANGE_POLICY", voice_profile="urgent", priority=4
            )
    ...
    def validate_config(self):
        ...
        try:
            ...
            if "sac_range" not in self.config:
                error_msg = "Clé 'sac_range' manquante dans la configuration"
                logger.error(error_msg)
            ...
            missing_keys = [
                key for key in required_keys if key not in self.config["sac_range"]
            ]
            if missing_keys:
                error_msg = f"Clés manquantes dans 'sac_range': {missing_keys}"
                logger.error(error_msg)
                raise ValueError(error_msg)
            ...
            self.save_snapshot("validate_config", {"config": self.config["sac_range"]})
        except Exception as e:
            latency = (datetime.now() - start_time).total_seconds()
    ...
    def log_performance(
        ...
        Args:
            operation (str): Nom de l’opération (ex. : create_range_policy).
        ...
        try:
            ...
            if memory_usage > 1024:
                ...
                miya_alerts(error_msg, tag="RANGE_POLICY", level="error", priority=5)
                AlertManager().send_alert(error_msg, priority=4)
                send_telegram_alert(error_msg)
            log_entry = {
            ...
            miya_alerts(error_msg, tag="RANGE_POLICY", level="error", priority=3)
    ...
    def save_snapshot(self, snapshot_type: str, data: Dict) -> None:
        ...
        Args:
            snapshot_type (str): Type de snapshot (ex. : create_range_policy).
    ...
    def create_range_policy(self) -> None:
        """
        Crée une politique SAC pour le mode range et la sauvegarde dans range_policy.pkl.
        """
        start_time = datetime.now()
        try:
            # Définir les chemins
            ...
            sac_config = self.config["sac_range"]
            model = SAC(
                policy="MlpPolicy",
                env=env,
            ...
            logger.info("Modèle SAC initialisé avec hyperparamètres pour mode range.")
            miya_speak(
                "Modèle SAC initialisé", tag="RANGE_POLICY", level="info", priority=2
            )
            ...
            latency = (datetime.now() - start_time).total_seconds()
            self.log_performance(
                "create_range_policy", latency, success=True, timesteps=total_timesteps
            )
            self.save_snapshot(
                "create_range_policy",
                {
            ...
            print(f"Politique range créée avec succès à {POLICY_PATH}")
        except Exception as e:
            latency = (datetime.now() - start_time).total_seconds()
            error_msg = f"Erreur lors de la création de la politique : {str(e)}\n{traceback.format_exc()}"
            logger.error(error_msg)
            miya_alerts(
                error_msg, tag="RANGE_POLICY", voice_profile="urgent", priority=5
            )
            AlertManager().send_alert(error_msg, priority=4)
            send_telegram_alert(error_msg)
            self.log_performance(
                "create_range_policy", latency, success=False, error=str(e)
            )
            self.save_snapshot("create_range_policy", {"error": str(e)})
            raise
...
def main():
    ...
    """
    try:
        generator = RangePolicyGenerator()
        ...
        print(error_msg)
        miya_alerts(error_msg, tag="RANGE_POLICY", voice_profile="urgent", priority=5)
        AlertManager().send_alert(error_msg, priority=4)
        send_telegram_alert(error_msg)
        logger.error(error_msg)
        exit(1)
