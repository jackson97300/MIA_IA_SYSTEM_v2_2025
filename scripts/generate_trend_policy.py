# -*- coding: utf-8 -*-
# MIA_IA_SYSTEM_v2_2025/src/scripts/generate_trend_policy.py
# Script pour générer la politique SAC pour le mode trend de MIA_IA_SYSTEM_v2_2025.
#
# Version : 2.1.3
# Date : 2025-05-13
#
# Rôle : Crée une politique SAC initiale pour le trading ES en mode trend (Phase 10).
...
#
# Dépendances : os, logging, pickle, pathlib, stable_baselines3>=2.0.0, gym>=0.21.0, numpy>=1.23.0,
...
#
# Outputs : src/model/router/policies/trend_policy.pkl,
#           data/logs/market/generate_trend_policy.log,
#           data/logs/generate_trend_policy_performance.csv,
#           data/trend_policy_snapshots/snapshot_*.json.gz
#
# Notes :
...
# - Tests unitaires dans tests/test_generate_trend_policy.py.
...
POLICY_PATH = os.path.join(
    BASE_DIR, "src", "model", "router", "policies", "trend_policy.pkl"
)
LOG_DIR = os.path.join(BASE_DIR, "data", "logs", "market")
SNAPSHOT_DIR = os.path.join(BASE_DIR, "data", "trend_policy_snapshots")
CSV_LOG_PATH = os.path.join(
    BASE_DIR, "data", "logs", "generate_trend_policy_performance.csv"
)

# Configuration du logging
os.makedirs(LOG_DIR, exist_ok=True)
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "generate_trend_policy.log"),
    level=logging.INFO,
...
class TrendPolicyGenerator:
    """
    Classe pour générer et sauvegarder une politique SAC pour le mode trend.
    """
    ...
    def __init__(self, config_path: Path = Path(CONFIG_PATH)):
        ...
        try:
            ...
            miya_speak(
                "TrendPolicyGenerator initialisé",
                tag="TREND_POLICY",
                voice_profile="calm",
                priority=2,
            )
            AlertManager().send_alert("TrendPolicyGenerator initialisé", priority=1)
            logger.info("TrendPolicyGenerator initialisé")
            self.log_performance("init", 0, success=True)
            self.save_snapshot("init", {"config_path": str(self.config_path)})
        except Exception as e:
            error_msg = f"Erreur initialisation TrendPolicyGenerator: {str(e)}\n{traceback.format_exc()}"
            miya_alerts(
                error_msg, tag="TREND_POLICY", voice_profile="urgent", priority=4
            )
    ...
    def validate_config(self):
        ...
        try:
            ...
            if "sac_trend" not in self.config:
                error_msg = "Clé 'sac_trend' manquante dans la configuration"
                logger.error(error_msg)
            ...
            missing_keys = [
                key for key in required_keys if key not in self.config["sac_trend"]
            ]
            if missing_keys:
                error_msg = f"Clés manquantes dans 'sac_trend': {missing_keys}"
                logger.error(error_msg)
                raise ValueError(error_msg)
            ...
            self.save_snapshot("validate_config", {"config": self.config["sac_trend"]})
        except Exception as e:
            latency = (datetime.now() - start_time).total_seconds()
    ...
    def log_performance(
        ...
        Args:
            operation (str): Nom de l’opération (ex. : create_trend_policy).
        ...
        try:
            ...
            if memory_usage > 1024:
                ...
                miya_alerts(error_msg, tag="TREND_POLICY", level="error", priority=5)
                AlertManager().send_alert(error_msg, priority=4)
                send_telegram_alert(error_msg)
            log_entry = {
            ...
            miya_alerts(error_msg, tag="TREND_POLICY", level="error", priority=3)
    ...
    def save_snapshot(self, snapshot_type: str, data: Dict) -> None:
        ...
        Args:
            snapshot_type (str): Type de snapshot (ex. : create_trend_policy).
    ...
    def create_trend_policy(self) -> None:
        """
        Crée une politique SAC pour le mode trend et la sauvegarde dans trend_policy.pkl.
        """
        start_time = datetime.now()
        try:
            # Définir les chemins
            ...
            sac_config = self.config["sac_trend"]
            model = SAC(
                policy="MlpPolicy",
                env=env,
            ...
            logger.info("Modèle SAC initialisé avec hyperparamètres pour mode trend.")
            miya_speak(
                "Modèle SAC initialisé", tag="TREND_POLICY", level="info", priority=2
            )
            ...
            latency = (datetime.now() - start_time).total_seconds()
            self.log_performance(
                "create_trend_policy", latency, success=True, timesteps=total_timesteps
            )
            self.save_snapshot(
                "create_trend_policy",
                {
            ...
            print(f"Politique trend créée avec succès à {POLICY_PATH}")
        except Exception as e:
            latency = (datetime.now() - start_time).total_seconds()
            error_msg = f"Erreur lors de la création de la politique : {str(e)}\n{traceback.format_exc()}"
            logger.error(error_msg)
            miya_alerts(
                error_msg, tag="TREND_POLICY", voice_profile="urgent", priority=5
            )
            AlertManager().send_alert(error_msg, priority=4)
            send_telegram_alert(error_msg)
            self.log_performance(
                "create_trend_policy", latency, success=False, error=str(e)
            )
            self.save_snapshot("create_trend_policy", {"error": str(e)})
            raise
...
def main():
    ...
    """
    try:
        generator = TrendPolicyGenerator()
        generator.create_trend_policy()
    except Exception as e:
        error_msg = (
            f"Échec de la création de la politique : {str(e)}\n{traceback.format_exc()}"
        )
        print(error_msg)
        miya_alerts(error_msg, tag="TREND_POLICY", voice_profile="urgent", priority=5)
        AlertManager().send_alert(error_msg, priority=4)
        send_telegram_alert(error_msg)
        logger.error(error_msg)
        exit(1)
