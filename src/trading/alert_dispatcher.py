...
class AlertDispatcher:
    def __init__(self, config: Dict):
        ...
        logger.warning("Client Discord initialisé. Assurez-vous que discord_client.start() est appelé, ou passez à un webhook pour une gestion simplifiée.")
    ...
    def log_alert_metadata(self, message: str, priority: int) -> None:
        ...
        try:
            alert_metadata = {
                ...
                "priority": priority,
                "channels": [c for c in [
                    "telegram" if priority >= 4 else None,
                    "discord" if priority >= 3 else None,
                    "email" if priority >= 5 else None
                ] if c is not None],  # Filtrer les None
            }
