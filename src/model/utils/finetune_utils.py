# -*- coding: utf-8 -*-
# MIA_IA_SYSTEM_v2_2025/src/model/utils/finetune_utils.py
# Gère le fine-tuning et l’apprentissage en ligne pour train_sac.py.
#
# Version : 2.1.4
# Date : 2025-05-13
#
# Rôle :
# Implémente le fine-tuning (méthode 8) et l’apprentissage en ligne (méthode 10) pour les modèles SAC,
# utilisant des mini-batchs et intégrant les 350 features pour l’entraînement et 150 SHAP features pour l’inférence.
# Intègre des mécanismes robustes (retries, validation, snapshots, logs psutil) pour une fiabilité optimale.
#
# Dépendances :
...
#
# Inputs :
# - config/es_config.yaml (paramètres de fine-tuning)
...
#
# Outputs :
# - Logs dans data/logs/finetune_utils.log
# - Logs de performance dans data/logs/finetune_performance.csv
# - Snapshots JSON compressés dans data/cache/finetune/<market>/*.json.gz
# - Sauvegardes incrémentielles dans data/checkpoints/finetune/<market>/*.json.gz
# - Modèles fine-tunés dans model/sac_models/<market>/<mode>/<policy_type>/
#
# Notes :
# - Intègre fine-tuning (méthode 8) avec mini-batchs et apprentissage en ligne (méthode 10) pour mises à jour en temps réel.
...
# - Tests unitaires disponibles dans tests/test_finetune_utils.py.
...
CACHE_DIR = BASE_DIR / "data" / "cache" / "finetune"
CHECKPOINT_DIR = BASE_DIR / "data" / "checkpoints" / "finetune"
LOG_DIR.mkdir(exist_ok=True)
CACHE_DIR.mkdir(exist_ok=True)
CHECKPOINT_DIR.mkdir(exist_ok=True)
logger.add(
    LOG_DIR / "finetune_utils.log", rotation="10 MB", level="INFO", encoding="utf-8"
)
logger.add(lambda msg: print(msg, end=""), level="INFO")

# Constantes
PERF_LOG_PATH = LOG_DIR / "finetune_performance.csv"
...
# Cache global pour les résultats de fine-tuning
finetune_cache = OrderedDict()
...
def log_performance(
    ...
    **kwargs,
    ...
    """
    Enregistre les performances (CPU, mémoire, latence) dans finetune_performance.csv.
    ...
    try:
        ...
        if memory_usage > 1024:
            ...
            miya_alerts(alert_msg, tag="FINETUNE", priority=5)
        ...
        miya_alerts(error_msg, tag="FINETUNE", priority=3)
...
def save_snapshot(
    ...
    Args:
        snapshot_type (str): Type de snapshot (ex. : finetune, online_learning).
    ...
    try:
        ...
        if file_size > 1.0:
            ...
            logger.warning(alert_msg)
            miya_alerts(alert_msg, tag="FINETUNE", priority=3)
        ...
        miya_speak(success_msg, tag="FINETUNE", voice_profile="calm")
...
def checkpoint(
    data: pd.DataFrame, data_type: str = "finetune_state", market: str = "ES"
) -> None:
    """
    ...
    Args:
        ...
        data_type (str): Type de données (ex. : finetune_state).
        market (str): Marché (ex. : ES, MNQ).
    """
    try:
        start_time = time.time()
        ...
        checkpoint_dir.mkdir(exist_ok=True)
        checkpoint_path = checkpoint_dir / f"finetune_{data_type}_{timestamp}.json.gz"
        checkpoint_versions = []
...
def cloud_backup(
    ...
    try:
        ...
        if not config.get("s3_bucket"):
            ...
            miya_alerts(warning_msg, tag="FINETUNE", priority=3)
            AlertManager().send_alert(warning_msg, priority=3)
            send_telegram_alert(warning_msg)
            return
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = (
            f"{config['s3_prefix']}finetune_{data_type}_{market}_{timestamp}.csv.gz"
        )
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt == max_attempts - 1:
                ...
                logger.error(error_msg)
                miya_alerts(error_msg, tag="FINETUNE", priority=4)
            ...
            logger.warning(warning_msg)
            miya_speak(warning_msg, tag="FINETUNE", level="warning")
            AlertManager().send_alert(warning_msg, priority=3)
            send_telegram_alert(warning_msg)
            time.sleep(delay)
...
def validate_features(
    ...
    try:
            ...
            if col in data.columns:
                ...
                if col in [
                    ...
                    if (data[col] <= 0).any():
                        ...
                        logger.warning(warning_msg)
                        miya_alerts(warning_msg, tag="FINETUNE", priority=4)
        ...
        logger.error(error_msg)
        miya_alerts(error_msg, tag="FINETUNE", priority=5)
...
def finetune_model(
    ...
    """
    Effectue le fine-tuning (méthode 8) d’un modèle SAC avec un mini-batch.

    Args:
        data (pd.DataFrame): Données contenant les 350 features (entraînement) ou 150 SHAP features (inférence).
        ent_coef (float): Coefficient d’entropie pour SAC. Par défaut 0.1.
        ...
        batch_size (int): Taille du mini-batch. Par défaut 100.
        total_timesteps (int): Nombre total de timesteps pour l’apprentissage. Par défaut 100.
        market (str): Marché (ex. : ES, MNQ).

    Returns:
        Optional[SAC]: Modèle SAC fine-tuné ou None si échec.
    """
    start_time = time.time()
    try:
        cache_key = f"{market}_{mode}_{policy_type}_{hash(str(data))}"
        if cache_key in finetune_cache:
            model = finetune_cache[cache_key]
            finetune_cache.move_to_end(cache_key)
            return model
        while len(finetune_cache) > MAX_CACHE_SIZE:
            finetune_cache.popitem(last=False)
        ...
        if len(data) < 1:
            raise ValueError(f"Données insuffisantes pour le fine-tuning pour {market}")

        # Charger le modèle SAC avec retries
        model_dir = MODEL_DIR / market / mode / policy_type

        def load_model():
            model_dir.mkdir(exist_ok=True)
            model_files = [f for f in model_dir.glob(f"sac_{mode}_*.zip")]
            if not model_files:
                raise FileNotFoundError(
                    f"Aucun modèle SAC trouvé dans {model_dir} pour {market}"
                )
            model_path = max(model_files, key=lambda x: x.stat().st_ctime)
            ...
            env.data = data
            return SAC.load(model_path, env=env), env

        result = with_retries(load_model, market=market)
        if result is None:
            raise RuntimeError(
                f"Échec du chargement du modèle après retries pour {market}"
            )
        model, env = result
        logger.info(f"Modèle SAC chargé pour fine-tuning: {model_dir}")

        # Sélectionner un mini-batch
        mini_batch = data.sample(n=min(batch_size, len(data)), random_state=42)
        env.data = mini_batch

        # Fine-tuning avec retries
        def perform_finetune():
            model.learn(total_timesteps=total_timesteps, ent_coef=ent_coef)
            return model

        model = with_retries(perform_finetune, market=market)
        if model is None:
            raise RuntimeError(f"Échec du fine-tuning après retries pour {market}")
        logger.info(
            f"Fine-tuning terminé pour mode {mode}, policy_type={policy_type}. CPU: {psutil.cpu_percent()}%"
        )
        ...
        logger.info(f"Modèle fine-tuné sauvegardé: {new_model_path}")
        ...
        save_snapshot("finetune_model", snapshot_data, market=market)
        checkpoint(mini_batch, data_type="finetune_batch", market=market)
        cloud_backup(mini_batch, data_type="finetune_batch", market=market)
        finetune_cache[cache_key] = model
        ...
        success_msg = f"Fine-tuning terminé pour {market}: mode={mode}, policy={policy_type}, batch_size={len(mini_batch)}"
        logger.info(success_msg)
        miya_speak(success_msg, tag="FINETUNE", voice_profile="calm")
        AlertManager().send_alert(success_msg, priority=1)
        send_telegram_alert(success_msg)
        ...
        error_msg = f"Erreur dans finetune_model pour {market}: {str(e)}\n{traceback.format_exc()}"
        logger.error(error_msg)
        miya_alerts(error_msg, tag="FINETUNE", priority=5)
        AlertManager().send_alert(error_msg, priority=5)
        send_telegram_alert(error_msg)
...
def online_learning(
    ...
    """
    Effectue l’apprentissage en ligne (méthode 10) pour mettre à jour un modèle SAC en temps réel.

    Args:
        data (pd.DataFrame): Nouvelles données pour l’apprentissage (350 features ou 150 SHAP features).
        model (SAC): Modèle SAC à mettre à jour.
        env (TradingEnv): Environnement de trading.
        batch_size (int): Taille du mini-batch. Par défaut 32.
        total_timesteps (int): Nombre total de timesteps pour l’apprentissage. Par défaut 50.
        learning_rate (float): Taux d’apprentissage. Par défaut 0.0001.
        config_path (str): Chemin vers la configuration. Par défaut "config/es_config.yaml".
        market (str): Marché (ex. : ES, MNQ).

    Returns:
        Optional[SAC]: Modèle SAC mis à jour ou None si échec.
    """
    start_time = time.time()
    try:
        # Valider les données
        validate_features(
            data, shap_features=(env.policy_type == "transformer"), market=market
        )
        if len(data) < 1:
            raise ValueError(
                f"Données insuffisantes pour l’apprentissage en ligne pour {market}"
            )
        ...
        def update_learning_rate():
            model.set_parameters({"learning_rate": learning_rate})
            return model

        model = with_retries(update_learning_rate, market=market)
        if model is None:
            raise RuntimeError(
                f"Échec de la mise à jour du taux d’apprentissage après retries pour {market}"
            )

        # Apprentissage en ligne avec retries
        def perform_learning():
            model.learn(total_timesteps=total_timesteps)
            return model

        model = with_retries(perform_learning, market=market)
        if model is None:
            raise RuntimeError(
                f"Échec de l’apprentissage en ligne après retries pour {market}"
            )
        logger.info(
            f"Apprentissage en ligne terminé pour mode {env.mode}, policy_type={env.policy_type}. CPU: {psutil.cpu_percent()}%"
        )

        # Sauvegarder le modèle mis à jour
        model_dir = MODEL_DIR / market / env.mode / env.policy_type
        model_dir.mkdir(exist_ok=True)
        new_model_path = (
            model_dir
            / f"sac_{env.mode}_{env.policy_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        )
        model.save(new_model_path)
        logger.info(f"Modèle mis à jour sauvegardé: {new_model_path}")
        ...
        save_snapshot("online_learning", snapshot_data, market=market)
        checkpoint(mini_batch, data_type="online_learning_batch", market=market)
        cloud_backup(mini_batch, data_type="online_learning_batch", market=market)
        ...
        success_msg = f"Apprentissage en ligne terminé pour {market}: mode={env.mode}, policy={env.policy_type}, batch_size={len(mini_batch)}"
        logger.info(success_msg)
        miya_speak(success_msg, tag="FINETUNE", voice_profile="calm")
        AlertManager().send_alert(success_msg, priority=1)
        send_telegram_alert(success_msg)
        ...
        error_msg = f"Erreur dans online_learning pour {market}: {str(e)}\n{traceback.format_exc()}"
        logger.error(error_msg)
        miya_alerts(error_msg, tag="FINETUNE", priority=5)
        AlertManager().send_alert(error_msg, priority=5)
        send_telegram_alert(error_msg)
