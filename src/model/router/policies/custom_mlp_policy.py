# -*- coding: utf-8 -*-
# MIA_IA_SYSTEM_v2_2025/src/model/router/policies/custom_mlp_policy.py
# Politique MLP personnalisée pour SAC/PPO/DDPG, conçue pour le routage avec attention contextuelle et régularisation dynamique.
#
# Version : 2.1.4
# Date : 2025-05-13
#
# Rôle :
# Implémente une politique MLP pour SAC/PPO/DDPG, gérant 350 features en entraînement et 150 SHAP features en production.
# Intègre l’attention contextuelle (méthode 9) basée sur le régime et la régularisation dynamique (méthode 14) via dropout
...
# - Observations (350 features en entraînement, 150 SHAP features en production)
#
# Outputs :
# - Actions prédites, valeurs, log probs
# - Logs dans data/logs/mlp_policy.log
# - Logs de performance dans data/logs/mlp_performance.csv
# - Snapshots compressés dans data/cache/mlp/*.json.gz
# - Sauvegardes incrémentielles dans data/checkpoints/mlp_*.json.gz
# - Visualisations dans data/figures/mlp/
#
# Notes :
# - Intègre attention contextuelle (méthode 9) et régularisation dynamique (méthode 14).
# - Valide 350 features pour l’entraînement et 150 SHAP features pour l’inférence via config/feature_sets.yaml.
...
# - Tests unitaires disponibles dans tests/test_custom_mlp_policy.py.
...
CACHE_DIR = BASE_DIR / "data" / "cache" / "mlp"
CHECKPOINT_DIR = BASE_DIR / "data" / "checkpoints"
FIGURE_DIR = BASE_DIR / "data" / "figures" / "mlp"
PERF_LOG_PATH = LOG_DIR / "mlp_performance.csv"
LOG_DIR.mkdir(exist_ok=True)
CACHE_DIR.mkdir(exist_ok=True)
CHECKPOINT_DIR.mkdir(exist_ok=True)
FIGURE_DIR.mkdir(exist_ok=True)
logger.add(LOG_DIR / "mlp_policy.log", rotation="10 MB", level="INFO", encoding="utf-8")
...
class CustomMLPPolicy(ActorCriticPolicy):
    """
    Politique MLP personnalisée pour SAC/PPO/DDPG avec attention contextuelle et régularisation dynamique.

    Attributes:
        observation_space (spaces.Box): Espace d’observation (350 ou 150 features).
        action_space (spaces.Box): Espace d’action.
        lr_schedule (Callable): Programme d’apprentissage.
        net_arch (List[int]): Architecture du réseau MLP.
        activation_fn (Type[nn.Module]): Fonction d’activation.
        normalize_inputs (bool): Si True, normalise les observations.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialise la politique MLP personnalisée.

        Args:
            observation_space (spaces.Box): Espace d’observation (350 pour entraînement, 150 pour production).
            action_space (spaces.Box): Espace d’action.
            lr_schedule (Callable): Fonction pour ajuster le taux d’apprentissage.
            net_arch (List[int], optional): Architecture MLP (défaut : [128, 64] en fast_mode, [256, 256, 128] sinon).
            activation_fn (Type[nn.Module]): Fonction d’activation (défaut : nn.ReLU).
            normalize_inputs (bool): Si True, normalise les observations (défaut : True).
            fast_mode (bool): Si True, utilise une architecture légère (défaut : False).
            training_mode (bool): Si True, utilise 350 features; sinon, 150 SHAP features (défaut : True).
        ...
        try:
            ...
            self.feature_dim = 350 if training_mode else 150
            net_arch = net_arch or ([128, 64] if self.fast_mode else [256, 256, 128])
            ...
            if len(expected_features) != self.feature_dim:
                raise ValueError(
                    f"Nombre de features incorrect dans feature_sets.yaml: {len(expected_features)} != {self.feature_dim}"
                )

            if observation_space.shape[0] != self.feature_dim:
                raise ValueError(
                    f"Observation space doit avoir {self.feature_dim} features, trouvé {observation_space.shape[0]}"
                )
            ...
            # Construction de l’extracteur MLP
            self._build_mlp_extractor()

            success_msg = (
                f"CustomMLPPolicy initialisée avec {self.feature_dim} features, "
                f"architecture: {self.net_arch}, normalize_inputs: {self.normalize_inputs}, "
                f"fast_mode: {self.fast_mode}, training_mode: {self.training_mode}"
            )
            ...
            self.log_performance(
                "init", latency, success=True, num_features=self.feature_dim
            )
            self.save_snapshot(
                "init",
                {
            ...
            error_msg = f"Erreur initialisation CustomMLPPolicy: {str(e)}\n{traceback.format_exc()}"
    ...
    def checkpoint(self, data: pd.DataFrame, data_type: str = "policy_state") -> None:
        ...
        try:
            ...
            checkpoint_path = CHECKPOINT_DIR / f"mlp_{data_type}_{timestamp}.json.gz"
            CHECKPOINT_DIR.mkdir(exist_ok=True)
    ...
    def cloud_backup(self, data: pd.DataFrame, data_type: str = "policy_state") -> None:
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_path = (
                f"{self.config['s3_prefix']}mlp_{data_type}_{timestamp}.csv.gz"
            )
    ...
    def log_performance(
        ...
        """
        Journalise les performances dans mlp_performance.csv.
    ...
    def _build_mlp_extractor(self) -> None:
        """
        Construit l’extracteur MLP avec dropout dynamique.
        """
        start_time = datetime.now()
        try:
            ...
            policy_layers = []
            value_layers = []
            for i, layer_size in enumerate(self.net_arch):
                policy_layers.append(
                    nn.Linear(
                        self.feature_dim if i == 0 else self.net_arch[i - 1], layer_size
                    )
                )
                policy_layers.append(self.activation_fn())
                policy_layers.append(
                    nn.Dropout(p=0.2)
                )  # Dropout initial, ajusté dynamiquement
                value_layers.append(
                    nn.Linear(
                        self.feature_dim if i == 0 else self.net_arch[i - 1], layer_size
                    )
                ...
                value_layers.append(nn.Dropout(p=0.2))
            self.mlp_extractor["policy_net"] = nn.Sequential(*policy_layers)
            self.mlp_extractor["value_net"] = nn.Sequential(*value_layers)
            success_msg = f"Extracteur MLP construit avec succès, input_dim={self.feature_dim}, fast_mode={self.fast_mode}"
            ...
            error_msg = (
                f"Erreur dans _build_mlp_extractor: {str(e)}\n{traceback.format_exc()}"
            )
            logger.error(error_msg)
            self.alert_manager.send_alert(error_msg, priority=4)
            send_telegram_alert(error_msg)
            self.log_performance("build_mlp_extractor", 0, success=False, error=str(e))
            raise

    def _validate_observations(self, obs: torch.Tensor) -> None:
        """
        Valide les observations.

        Args:
            obs (torch.Tensor): Tenseur d’observations.
        """
        start_time = datetime.now()
        try:

            def validate():
                expected_dim = 350 if self.training_mode else 150
                if len(obs.shape) != 2 or obs.shape[1] != expected_dim:
                    raise ValueError(
                        f"Observation doit être de forme (batch_size, {expected_dim}), trouvé {obs.shape}"
                    )
                ...
                confidence_drop_rate = (
                    (nan_count + inf_count) / (obs.shape[0] * obs.shape[1])
                    if (obs.shape[0] * obs.shape[1]) > 0
                    else 0.0
                )
    ...
    def update_normalization_stats(self, obs: torch.Tensor) -> None:
        ...
        try:
            ...
            if obs.shape[-1] != expected_dim:
                raise ValueError(
                    f"Taille des observations incorrecte: {obs.shape[-1]} au lieu de {expected_dim}"
                )
            if self.normalize_inputs:
                obs_np = obs.cpu().numpy()
                self.obs_window.extend(obs_np)
    ...
    def update_performance_thresholds(
        ...
        Args:
            ...
            context (Optional[Dict]): Contexte avec métriques (ex. : vix, spread, regime_probs).
        """
        start_time = datetime.now()
        try:
    ...
    def forward(
        ...
        """
        Effectue un passage avant avec attention contextuelle et régularisation dynamique.

        Args:
            obs (torch.Tensor): Tenseur d’observations (batch_size, 350 ou 150).
            deterministic (bool): Si True, utilise la moyenne des actions.
            context (Optional[Dict]): Contexte avec vix, spread, regime_probs, etc.

        Returns:
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor]: Actions, valeurs, log probs.
        """
        start_time = datetime.now()
        try:
            ...
            def compute_forward():
                ...
                regime_probs = (
                    ...
                    if context
                    else {"trend": 0.4, "range": 0.4, "defensive": 0.2}
                )
                ...
                if self.normalize_inputs:
                    ...
                    if self.obs_window:
                        ...
                        obs = torch.tensor(
                            obs_normalized, dtype=torch.float32, device=obs.device
                        )
                ...
                if ultra_defensive:
                    ...
                    regime_weights = {
                        k: regime_probs.get(k, 0.0)
                        for k in ["trend", "range", "defensive"]
                    }
                    for i in range(obs.shape[1]):
                        feature_weight = weights.get(f"feature_{i}", 1.0)
                        regime_factor = sum(
                            regime_weights[reg] * (1.2 if reg == regime else 0.8)
                            for reg in regime_weights
                        )
                        obs[:, i] *= feature_weight * regime_factor
                    ...
                    cache_key = f"{obs.sum().item()}_{deterministic}_{regime}"
                    if cache_key in self.prediction_cache:
                        actions, values, log_probs, confidence_score = (
                ...
                plt.plot(
                    actions.cpu().numpy().flatten(), label="Actions", color="green"
                )
                plt.plot(
                    obs.mean(dim=0).cpu().numpy(),
                ...
                plt.title(
                    f"MLP Policy: Regime {regime}, Action Mean: {actions.mean().item():.2f}, Dropout: {dropout:.2f}"
                )
                plt.xlabel("Index")
                plt.ylabel("Value")
                plt.legend()
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                plt.savefig(FIGURE_DIR / f"actions_{timestamp}.png")
                plt.close()
    ...
    def _get_constructor_parameters(self) -> Dict[str, Any]:
        """
        Retourne les paramètres pour la reconstruction.
