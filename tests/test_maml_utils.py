# -*- coding: utf-8 -*-
# MIA_IA_SYSTEM_v2_2025/tests/test_maml_utils.py
# Tests unitaires pour src/model/utils/maml_utils.py
#
# Version : 2.1.4
# Date : 2025-05-13
#
# Rôle : Valide le meta-learning (méthode 18) avec prototypical
# networks pour adapter les modèles SAC, utilisant des mini-batchs et
# intégrant les 350 features pour l’entraînement et 150 SHAP features
# pour l’inférence, avec support multi-marchés.
#
# Dépendances :
# - pytest>=7.3.0,<8.0.0
# - pandas>=2.0.0,<3.0.0
...
#
# Outputs :
# - Tests unitaires validant les fonctionnalités de maml_utils.py.
#
# Notes :
...
from src.model.utils.maml_utils import (
...
def tmp_dirs(tmp_path):
    """Crée des répertoires temporaires pour les logs, cache,
    checkpoints, et modèles."""
    ...
    logs_dir.mkdir()
    cache_dir = data_dir / "cache" / "maml" / "ES"
    cache_dir.mkdir(parents=True)
    checkpoints_dir = data_dir / "checkpoints" / "maml" / "ES"
    ...
    return {
        ...
        "perf_log_path": str(logs_dir / "maml_performance.csv"),
    }
...
@pytest.mark.asyncio
async def test_apply_prototypical_networks(tmp_dirs, mock_data, mock_model):
    """Teste l’application du meta-learning avec prototypical networks."""
    with patch(
        ...
        mock_env_instance.mode = "trend"
        mock_env_instance.policy_type = "mlp"
        mock_env_instance.sequence_length = 50
        mock_env_instance.obs_t = [f"feature_{i}" for i in range(350)]
        mock_env_instance.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(350,),
            dtype=np.float32,
        )
        mock_env_instance.data = mock_data
        ...
        assert model is not None, "Modèle adapté non retourné"
        snapshot_files = os.listdir(tmp_dirs["cache_dir"])
        assert any(
            "snapshot_apply_prototypical_networks" in f
            and f.endswith(".json.gz")
            for f in snapshot_files
        ), "Snapshot apply_prototypical_networks non créé"
        checkpoint_files = os.listdir(tmp_dirs["checkpoints_dir"])
        assert any(
            "maml_batch" in f and f.endswith(".json.gz")
            for f in checkpoint_files
        ), "Checkpoint maml_batch non créé"
        df_perf = pd.read_csv(tmp_dirs["perf_log_path"])
        assert any(
            "apply_prototypical_networks" in str(op)
            for op in df_perf["operation"]
        ), "Opération apply_prototypical_networks non journalisée"
        mock_telegram.assert_called()
...
async def test_cloud_backup(tmp_dirs):
    ...
    with patch("boto3.client") as mock_s3, patch(
        ...
        from src.model.utils.maml_utils import cloud_backup
...
async def test_checkpoint(tmp_dirs):
    ...
    with patch("src.utils.telegram_alert.send_telegram_alert") as mock_telegram:
        ...
        from src.model.utils.maml_utils import checkpoint
        ...
        checkpoint_files = os.listdir(tmp_dirs["checkpoints_dir"])
        assert any(
            "maml_test_metrics" in f and f.endswith(".json.gz")
            for f in checkpoint_files
        ), "Checkpoint non créé"
        with gzip.open(
