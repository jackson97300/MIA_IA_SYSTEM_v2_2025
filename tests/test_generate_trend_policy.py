# -*- coding: utf-8 -*-
# MIA_IA_SYSTEM_v2_2025/tests/test_generate_trend_policy.py
# Tests unitaires pour generate_trend_policy.py.
#
# Version : 2.1.3
# Date : 2025-05-13
#
# Rôle : Vérifie les fonctionnalités de TrendPolicyGenerator, incluant l'initialisation,
...
# - psutil>=5.9.8
# - pandas>=2.0.0
# - src.scripts.generate_trend_policy
...
from src.scripts.generate_trend_policy import TrendPolicyGenerator
...
def temp_dir(tmp_path):
    ...
    snapshots_dir = data_dir / "trend_policy_snapshots"
...
def generator(temp_dir, mock_config, monkeypatch):
    """Initialise TrendPolicyGenerator avec des mocks."""
    monkeypatch.setattr(
        "src.scripts.generate_trend_policy.CONFIG_PATH", str(mock_config)
    )
    monkeypatch.setattr(
        "src.scripts.generate_trend_policy.config_manager.get_config",
        lambda x: {
            "sac_trend": {
                "learning_rate": 0.0003,
                "buffer_size": 1000000,
    ...
    generator = TrendPolicyGenerator(config_path=mock_config)
    return generator


def test_init_generator(temp_dir, mock_config, monkeypatch):
    """Teste l'initialisation de TrendPolicyGenerator."""
    generator = TrendPolicyGenerator(config_path=mock_config)
    assert generator.config_path == mock_config
    assert generator.config["sac_trend"]["learning_rate"] == 0.0003
    assert os.path.exists(temp_dir / "data" / "trend_policy_snapshots")
    snapshots = list(
        (temp_dir / "data" / "trend_policy_snapshots").glob("snapshot_init_*.json.gz")
    )
    assert len(snapshots) >= 1
    perf_log = (
        temp_dir / "data" / "logs" / "market" / "generate_trend_policy_performance.csv"
    )
...
def test_validate_config_invalid(temp_dir, monkeypatch):
    ...
    with pytest.raises(FileNotFoundError, match="Fichier de configuration introuvable"):
        TrendPolicyGenerator(config_path=invalid_config)

    with patch(
        "src.scripts.generate_trend_policy.config_manager.get_config", return_value={}
    ):
        with pytest.raises(ValueError, match="Clé 'sac_trend' manquante"):
            TrendPolicyGenerator(config_path=temp_dir / "config" / "es_config.yaml")


def test_create_trend_policy_success(temp_dir, generator, monkeypatch):
    ...
    policy_path = (
        temp_dir / "src" / "model" / "router" / "policies" / "trend_policy.pkl"
    )
    assert policy_path.exists()
    snapshots = list(
        (temp_dir / "data" / "trend_policy_snapshots").glob(
            "snapshot_create_trend_policy_*.json.gz"
        )
    )
    assert len(snapshots) >= 1
    perf_log = (
...
def test_create_trend_policy_failure(temp_dir, generator, monkeypatch):
